-- Add server-side plausibility guards to game result RPCs to make
-- client-reported result tampering harder. Pure plausibility checks; no
-- rate limiting (per platform directive).

CREATE OR REPLACE FUNCTION public.process_game_result(p_wave integer, p_gold integer, p_victory boolean)
 RETURNS TABLE(gems integer, high_score integer, score integer, earned integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  -- Plausibility: a victorious run must have made some progress
  IF COALESCE(p_victory,false) AND v_wave < 1 THEN
...
CREATE OR REPLACE FUNCTION public.process_phase_result(
  p_wave integer, p_gold integer, p_victory boolean,
  p_stage_id integer, p_phase_index integer, p_took_damage boolean,
  p_towers_used integer, p_phase_seconds integer, p_enemies_killed integer,
  p_gold_spent integer, p_used_upgrade boolean, p_stage_complete boolean,
...
BEGIN
  ...
  -- Plausibility guards (server-authoritative sanity checks)
